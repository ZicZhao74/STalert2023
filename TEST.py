...
def gethislist_stock_no():
    hislist = pd.read_csv(
        path + '/historydata/onestock.csv', encoding='utf-8-sig')
    stock_nolist = []
...
data.to_csv(path+date+"stock_fiancing.csv",
            index=False, encoding='utf-8-sig')
data = data.drop(columns=["融資買進", "融資賣出", "融資現金償還", "融資限額",
                 "融券買進", "融券賣出", "融券現券償還", "融券限額", "資券互抵", "註記"], axis=1)
print(data)
...
for i in range(0, len(stock_no_list)):
    ...
    thedata.to_csv(path+"/historydata/"+stock_no +
                   "stock_fiancing.csv", encoding='utf-8-sig')
