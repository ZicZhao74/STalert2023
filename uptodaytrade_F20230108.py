...
strdate=str(date)
data.to_csv(path+"/daystock/"+strdate+'daystock.csv', index=False, encoding='utf-8-sig')
print (strdate)
#從歷史資料檔名列表取得股號
...
for i in range(0,len(stock_nolist)):#
    ...
    # 將各股今日交易結果聯集到歷史資料
    historydata = pd.read_csv(path+'/kdnewhistory/'+hislist.iat[i, 0],thousands=',',index_col=0)
        #如果有重複先DROP
    ...
    fianl=KD(final)
    #print(final)
    todir=path+'/kdnewhistory/'
    tofilename=hislist.iat[i, 0]
    final.to_csv(todir+tofilename,encoding='utf-8-sig')
