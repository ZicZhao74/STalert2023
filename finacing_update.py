...
for i in range(0,len(stock_nolist)):#
    #讀入歷史資料
    data=pd.read_csv(path+"/newfinancing/"+stock_nolist[i]+"stock_fiancing.csv",index_col=0)
    ...
    result.to_csv(path+"/newfinancing/"+stock_nolist[i]+"stock_fiancing.csv",encoding='utf-8-sig')
